...
yield_SNP_models <- data.frame(
  SNP = c(colnames(yield_BLUEs_dist)[3:7]),
...
yield_SNP_recode <- yield_BLUEs_dist %>%
  ...
  mutate(genotype_class = case_when(
      ...
      model == "1-dom-alt" & genotype != "AAAA" ~ "simplex dominant (---B)",
      model == "1-dom-ref" & genotype == "BBBB" ~ "recessive (BBBB)",
      model == "1-dom-ref" & genotype != "BBBB" ~ "simplex dominant (A---)",
      model == "2-dom-alt" & genotype == "AAAA" ~ "recessive (AAAA)",
      model == "2-dom-alt" & genotype != "AAAA" ~ "duplex dominant (--BB)"
    )) %>%
