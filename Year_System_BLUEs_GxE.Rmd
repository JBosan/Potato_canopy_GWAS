...
For the original BLUEs (i.e. not Year_System_GxE) this is done by setting up a mixed effect model where...

- Fixed effects = Trait of interest, Genotype (i.e. Trait ~ Genotype)
- Random effects = Cultivation system (System) and Year

For the Year_System_GxE BLUEs we shall also add 

- Gene x year as a random effect (Genotype:Year)
- AND Gene x system as a random effect (Genotype:system)
...
yield_lmer <- lmer(formula = Yield_Kg ~ Genotype_ID + (1|System) + (1|Year) + (1|Genotype_ID:Year) + (1|Genotype_ID:System),
                   data = phenotype,
                   REML = TRUE)
```
...
```{r, message=FALSE}
pdf(file = "Year_System_GxE_BLUEs_output/yield_qq.pdf")
...
Max_Max_height_m_lmer <- lmer(formula = Max_Max_height_m ~ Genotype_ID 
                         + (1|System) + (1|Year) + (1|Genotype_ID:Year) + (1|Genotype_ID:System),
...
```{r, message=FALSE}
pdf(file = "Year_System_GxE_BLUEs_output/Max_Max_height_m_qq.pdf")
...
```{r, message=FALSE}
pdf(file = "Year_System_GxE_BLUEs_output/Max_Ave_height_m_qq.pdf")
...
```{r, message=FALSE}
pdf(file = "Year_System_GxE_BLUEs_output/Max_area_plant_m2_qq.pdf")
...
```{r, message=FALSE}
pdf(file = "Year_System_GxE_BLUEs_output/Max_volume_plant_m3_qq.pdf")
...
write.csv(all_BLUEs, 
          "Year_System_GxE_BLUEs_output/Year_System_GxE_BLUEs.csv",
          row.names = FALSE)
```
...
write.csv(BLUEs_with_Q_matrices_and_PCs, 
          "Year_System_GxE_BLUEs_output/Year_System_GxE_BLUEs_with_Q_matrices_and_PCs.csv",
          row.names = FALSE)
